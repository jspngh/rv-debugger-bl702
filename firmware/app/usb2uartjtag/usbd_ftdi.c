    ...
    switch (pSetup->bRequest) {
        ...
        case SIO_SET_DATA_REQUEST:
            ...
            if (actual_baudrate != 1200) {
                ...
                //          actual_baudrate, (uint8_t)pSetup->wValue,
                //          parity_name[(uint8_t)(pSetup->wValue >> 8)],
                //          stop_name[(uint8_t)(pSetup->wValue >> 11)]);
                usbd_ftdi_set_line_coding(actual_baudrate, (uint8_t)pSetup->wValue,
                                          (uint8_t)(pSetup->wValue >> 8),
                                          (uint8_t)(pSetup->wValue >> 11));
            }
            break;
        ...
        case SIO_POLL_MODEM_STATUS_REQUEST:
             ...
             * device. The device sends these bytes also as a header for each
             * read access where they are discarded by ftdi_read_data().
             * The chip generates the two stripped status bytes
             * in the absence of data every 40 ms.
             *
