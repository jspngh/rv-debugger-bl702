...
// UART RX -> USB IN
uint16_t usb_dc_ftdi_send_from_ringbuffer(struct device *dev,
...
// USB OUT -> UART TX
int usb_dc_ftdi_receive_to_ringbuffer(struct device *dev, Ring_Buffer_Type *rb,
                                      uint8_t ep)
{
    uint8_t ep_idx;
    uint8_t recv_len;
...
int main(void)
{
    ...
    if (usb_fs) {
        device_control(usb_fs, DEVICE_CTRL_SET_INT,
                       (void *)(USB_SOF_IT | USB_EP2_DATA_OUT_IT |
                                USB_EP1_DATA_IN_IT | USB_EP4_DATA_OUT_IT |
                                USB_EP3_DATA_IN_IT));
    }
